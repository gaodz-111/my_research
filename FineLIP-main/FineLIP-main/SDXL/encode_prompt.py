...
def initialize(args):
    ...
    global bigG_model, bigG_encoder, openclip_tokenizer, vitl_model, clip_tokenizer

    if args.enable_finelip:
        context_length = 248
    else:
        context_length = 77
    ...
    bigG_model, _, bigG_preprocess = open_clip.create_model_and_transforms(
        ...
        text_cfg={'context_length': context_length, 'vocab_size': 49408, "width": 1280, "heads": 20, "layers": 32}
    )
    if args.enable_finelip:
    ...
    openclip_tokenizer = open_clip.get_tokenizer('ViT-bigG-14', context_length=context_length)

    device = "cuda" if torch.cuda.is_available() else "cpu"
    if args.enable_finelip:
        vitl_model, vitl_preprocess = finelip.load(args.ckpt_path, device=device)
        clip_tokenizer = finelip.tokenize
    else:
        vitl_model, vitl_preprocess = clip.load("ViT-L/14", device=device)
...
with torch.no_grad():
    def encode_prompt(
        ...
        if pipe.text_encoder is not None:
            ...
            pipe.text_encoder = vitl_model.encode_text_full
        ...
        if prompt_embeds is None:
            ...
            for prompt, tokenizer, text_encoder in zip(prompts, tokenizers, text_encoders):
                ...
                try: 
                    ...
                    text_inputs = tokenizer(prompt, truncate=True)
                    text_input_ids = text_inputs
                    untruncated_ids = tokenizer(prompt, truncate=True)
