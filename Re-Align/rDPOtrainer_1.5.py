...
class rDPOTrainer(DPOTrainer):
    ...
    def concatenated_forward(
        ...
        retrieved_images = batch.get("retrieved_images", None) 
        chosen_model_kwargs = (
            {
                "labels": chosen_label,
        ...
        # 确保new_chosen_labels的形状正确
        if new_chosen_labels.dim() == 3:  # 如果形状是 [batch_size, seq_len, hidden_size]
            # 取最后一个维度的最大值作为标签
            new_chosen_labels = new_chosen_labels.argmax(dim=-1)
        ...
        # 确保new_rejected_labels的形状正确
        if new_rejected_labels.dim() == 3:
            new_rejected_labels = new_rejected_labels.argmax(dim=-1)
        ...
        # 确保new_imageless_chosen_labels的形状正确
        if new_imageless_chosen_labels.dim() == 3:
            new_imageless_chosen_labels = new_imageless_chosen_labels.argmax(dim=-1)
