...
import torch
import transformers
from transformers import TrainerCallback, GenerationConfig,AutoModelForCausalLM
...
wandb.login(key="a233709c450a80561b84bfea81974fba41cbb549")
local_rank = None
...
class ModelArguments:
    model_name_or_path: Optional[str] = field(default="/data2/gaodz/llava-v1.6-vicuna-7b")
...
def train():
    ...
    # 添加调试信息，检查输入数据的形状
    def debug_input_shapes(batch):
        print("Debugging input shapes:")
        ...
        if 'images' in batch:
            ...
            print(f"retrieved_images shape: {batch['retrieved_images'].shape}")
    
    class CustomTrainer(LLaVArDPOTrainer):
        def training_step(self, model, inputs, num_items_in_batch=None):
            # 添加调试信息，检查输入数据的形状
            debug_input_shapes(inputs)
            
            # 检查输入张量的形状是否正确
            if inputs['chosen_input_ids'].size(1) == 0 or inputs['rejected_input_ids'].size(1) == 0:
                raise ValueError("Input tensor has zero length in dimension 1. Check data preprocessing.")
    ...
    if model_args.vision_tower is not None:
        if 'mpt' in model_args.model_name_or_path:
            ...
            # flat = {**config.text_config.to_dict(), **config.vision_config.to_dict()}
    ...
    if model_args.vision_tower is not None:
        ...
        if vision_tower is not None:
            ...
            # 修改这里：直接从 transformers 加载处理器
            from transformers import CLIPProcessor
            processor = CLIPProcessor.from_pretrained(model_args.vision_tower)
    ...
    if training_args.lora_enable:
        trainer = LLaVArDPOTrainer(  # 使用自定义的 Trainer 类
            model,
            args=training_args,
