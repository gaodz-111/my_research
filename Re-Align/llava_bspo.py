...
class CustomGRPOTrainer(GRPOTrainer):
    ...
    def _compute_loss(self, model, inputs):
        ...
        outputs = model(
            input_ids=completion_ids.view(-1, completion_ids.size(-1)),  # [B*N, L]
            attention_mask=completion_mask.view(-1, completion_mask.size(-1)),  # [B*N, L]
            images=images.repeat(num_generations, 1, 1, 1),  # [B*N, C, H, W]
            output_hidden_states=True,
            return_dict=True
        )
        # 取最后一个 token 的 hidden state
        last_hidden = outputs.hidden_states[-1]  # [B*N, L, D]
        cls_hidden = last_hidden[:, -1, :]  # [B*N, D]
        v_pred = model.value_head(cls_hidden).squeeze(-1)  # [B*N]

        # 2) 规则奖励 r，已经在 rewards_tensor 中
        r = rewards_tensor.view(-1)  # [B*N]

        # 3) 构造 Bellman 目标：TβV ≈ r
        value_loss = F.mse_loss(v_pred, r)

        # 4) 合入总 loss
        loss = loss + self.args.beta * value_loss
        return loss
...
def format_reward(completions, **kwargs):
    ...
    print("---format_score---", rewards_list)
    return rewards_list
...
#                     mask_answer = sp.get("judgment", "").strip()
#
...
#     print("---mask_score---",rewards)
#     return rewards
emb_model = SentenceTransformer('/data2/gaodz/bge_m3')

def compute_mask_match(completions, emb_model=emb_model, **kwargs):
    """
    使用向量语义相似度计算生成质量（用于object, attribute, action, description类型问题）。
    对每个样本返回一个reward得分。
    """
    if emb_model is None:
        raise ValueError("emb_model is required and must have .encode method.")
    ...
    for idx, completion in enumerate(completions):
        ...
        type_match = re.search(r"<type>\s*[$$]?(.*?)[\s]*[$$]?\s*</type>", content, re.DOTALL)
        if not type_match:
            rewards.append(0.0)
            continue
        types_needed = [t.strip() for t in type_match.group(1).split(",")]

        answer_match = re.search(r"<answer>\s*[$$]?(.*?)[\s]*[$$]?\s*</answer>", content, re.DOTALL)
        gen_answer_text = answer_match.group(1).strip() if answer_match else ""

        rethink_match = re.search(r"<rethink>\s*[$$]?(.*?)[\s]*[$$]?\s*</rethink>", content, re.DOTALL)
        gen_rethink_text = rethink_match.group(1).strip() if rethink_match else ""

        subpro_match = re.search(r"<subproblem>\s*[$$]?(.*?)[\s]*[$$]?\s*</subproblem>", content, re.DOTALL)
        gen_subpro_text = subpro_match.group(1).strip() if subpro_match else ""
        try:
            raw_data = raw_data_list[idx]
            sub_problems = raw_data.get("sub problem", [])
            raw_question = raw_data.get("question", "").strip()
            emb_raw_question = emb_model.encode(raw_question, convert_to_tensor=True)
        ...
        for q_type in types_needed:
            ...
            for sp in sub_problems:
                ...
                emb_sp_question = emb_model.encode(gen_subpro_text, convert_to_tensor=True)
                sim_question = util.cos_sim(emb_raw_question, emb_sp_question).item()
                ...
                ref_answer = sp.get("answer", "").strip()
                mask_answer = sp.get("judgment", "").strip()

                ref_answer = ref_answer.split("Original Answer:")[-1].strip()
                mask_answer = mask_answer.split("Original Answer:")[-1].strip()

                try:
                    emb_mask_answer = emb_model.encode(mask_answer, convert_to_tensor=True)
                    emb_gen_answer = emb_model.encode(gen_answer_text, convert_to_tensor=True)

                    emb_ref_answer = emb_model.encode(ref_answer, convert_to_tensor=True)
                    emb_gen_rethink = emb_model.encode(gen_rethink_text, convert_to_tensor=True)

                    sim_mask = util.cos_sim(emb_mask_answer, emb_gen_answer).item()
                    sim_rethink = util.cos_sim(emb_ref_answer, emb_gen_rethink).item()

                    combined_score = 0.4 * sim_mask + 0.6 * sim_rethink
                    sim_scores.append(combined_score)

                except Exception as e:
                    print(f"[Encoding error] {e}")
                    continue

        final_score = float(np.mean(sim_scores)) if sim_scores else 0.0
...
def compute_consistency(completions, emb_model=emb_model, **kwargs):
    """
    只处理 question_type in ["yesno", "count", "location"] 的子问题。
    返回每个样本的语义相似度平均得分（多子问题平均）。
    """
    if emb_model is None:
        raise ValueError("emb_model is required")
    ...
    for idx, completion in enumerate(completions):
        ...
        if not rethink_match:
            rewards.append(0.0)
            continue
        list_b = [rethink_match.group(1).strip()]
        ...
        for q_type in types_needed:
            ...
            for sp in sub_problems:
                if sp.get("question_type") == q_type:
                    ref = sp.get("rethink", "").strip()
                    filtered_answers.append(ref)
    ...
    print("---consistency_score---", rewards)
    return rewards
...
@dataclass
class DataArguments:
    data_path: str = field(default="/data2/gaodz/Re-Align/data_step7_cleaned_last.json",
...
if __name__ == "__main__":
    ...
    nn.init.xavier_uniform_(model.value_head.weight)
